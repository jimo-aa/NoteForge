...
impl SearchEngine {
    ...
    pub fn search(&self, query_str: &str, limit: usize) -> Result<Vec<SearchResult>, Box<dyn std::error::Error>> {
        ...
        let mut parsed = query_str.split_whitespace().collect::<Vec<_>>().join(" ");
        if parsed.is_empty() { parsed = query_str.to_string(); }
        let query = query_parser.parse_query(&parsed)?;
        ...
        for (score, addr) in top_docs {
            ...
            let id = retrieved.get_first(id_field).and_then(|v| v.as_str()).unwrap_or("").to_string();
            let title = retrieved.get_first(title_field).and_then(|v| v.as_str()).unwrap_or("").to_string();
            let updated_at = retrieved.get_first(updated_at_field).and_then(|v| v.as_u64()).unwrap_or_else(now_ms);
