...
export function Sidebar({ onNewNote }: SidebarProps) {
  const { searchQuery, setSearchQuery, currentFilter, setCurrentFilter, activeNotebook, setActiveNotebook, notebooks, tags, favoriteCount, totalCount, searchResultCount, setIsGraphOpen, selectNote, currentNoteId, activeTags, setActiveTags, filteredNotes, setContextMenu, openEntityModal } = useStore();
  ...
  return (
    <aside className="sidebar sidebar--compact">
      ...
      <section className="sidebar-section tags-section"><div className="section-heading static"><span>标签</span></div><div className="tag-grid">{visibleTags.length === 0 ? <div className="tag-empty"><div className="tag-empty__title">暂无标签</div><div className="tag-empty__desc">创建笔记并添加标签后，会在这里显示可筛选的标签。</div></div> : <>{visibleTags.map((tag) => { const selected = activeTags.includes(tag); return (<button key={tag} className={selected ? 'tag-pill active' : 'tag-pill'} onClick={() => { setActiveTags((prev) => prev.includes(tag) ? prev.filter((t) => t !== tag) : [...prev, tag]); setCurrentFilter('tag'); }} title={`#${tag}`}>#{tag}</button>); })}<div className="tag-empty tag-empty--inline"><div className="tag-empty__title">标签筛选</div><div className="tag-empty__desc">支持多选，未选中任何标签时将默认展示全部文件列表。</div></div></>}</div></section>
      <section className="sidebar-notes"><div className="section-heading static notes-heading"><span>最近笔记</span><span>{searchResultCount ?? totalCount} 条</span></div><div className="sidebar-note-scroll">{visibleNotes.length === 0 ? <div className="sidebar-empty">暂无笔记</div> : visibleNotes.map((note) => (<button key={note.meta.id} className={note.meta.id === currentNoteId ? 'sidebar-note active' : 'sidebar-note'} onClick={() => selectNote(note.meta.id)} onContextMenu={(e) => { e.preventDefault(); setContextMenu({ visible: true, x: e.clientX, y: e.clientY, noteId: note.meta.id, notebookId: null, kind: 'note' }); }}><div className="sidebar-note-title"><span>{note.meta.isPinned ? '📌' : note.meta.isFavorite ? '⭐' : ''}</span><strong>{note.meta.title}</strong></div><p>{note.content.replace(/[#*`>-]/g, '').slice(0, 42)}...</p><div className="sidebar-note-meta"><span>0分钟前</span></div></button>))}</div></section>
      <footer className="sidebar-status"><span className="sync-dot" /><span>已同步</span><span>{favoriteCount} 收藏</span></footer>
    </aside>
  );
}
