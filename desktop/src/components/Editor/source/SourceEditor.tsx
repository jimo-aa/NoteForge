// NoteForge — Source Editor (CodeMirror 6)
// Clean CodeMirror 6 wrapper extracted from the legacy CodeMirrorEditor.tsx.
// Supports search highlight, task checkboxes, image preview, wiki links.
...
// ── WYSIWYG decorations (task checkboxes, images, wiki links) ──
...
function computeWysiwygDecorations(state: EditorState): DecorationSet {
  ...
  while ((match = imgRe.exec(docStr)) !== null) {
    ...
    if (!src) continue;
    decorations.push(
      Decoration.replace({ widget: new ImagePreviewWidget(src, alt ?? '') }).range(match.index, match.index + match[0].length),
    );
  }
  const wikiRe = /\[\[([^\]]+)\]\]/g;
...
const wysiwygTheme = EditorView.baseTheme({
  ...
  '.cm-image-wrapper': { display: 'inline-block', verticalAlign: 'middle', margin: '4px 0' },
  '.cm-image-preview': { maxWidth: '100%', maxHeight: '300px', borderRadius: '4px', objectFit: 'contain', cursor: 'default' },
});

function createEditorState(doc: string, onChange: (text: string) => void, placeholderText: string) {
  return EditorState.create({
    ...
    extensions: [
      ...
      langComp.of(markdown({ base: markdownLanguage, codeLanguages: languages })),
...
export const SourceEditor = forwardRef<EditorHandle, SourceEditorProps>(
  function SourceEditor(
    { initialContent, onChange, onSelectionChange, onImagePaste, onImageDrop, onWikiLinkClick, placeholderText = '开始编写笔记...', searchQuery },
    ref,
  ) {
    ...
    useEffect(() => {
      ...
      if (initialContent !== currentDoc) {
        view.dispatch({ changes: { from: 0, to: currentDoc.length, insert: initialContent } });
      }
    }, [initialContent]);
    ...
    useImperativeHandle(ref, () => ({
      ...
      setSelection: (from: number, to: number) => {
        ...
        if (safeFrom === safeTo && safeFrom === 0) return;
      ...
      wrapText: (before: string, after: string) => {
        ...
        if (!view) return;
        const sel = view.state.selection.main;
        const selected = view.state.doc.slice(sel.from, sel.to).toString() || 'text';
        view.dispatch({ changes: { from: sel.from, to: sel.to, insert: `${before}${selected}${after}` }, selection: { anchor: sel.from + before.length + selected.length } });
        view.focus();
      },
    }), []);

    return (
      <div ref={containerRef} className="codemirror-wrapper" style={{ width: '100%', height: '100%', overflow: 'auto' }} />
    );
  },
);
