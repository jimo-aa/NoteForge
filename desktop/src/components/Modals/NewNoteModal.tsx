...
export function NewNoteModal({ open, notebooks = [], onClose, onCreate }: NewNoteModalProps) {
  ...
  const createNote = () => {
    ...
    onCreate({
      ...
      notebookId,
      tags,
      content: content.trim() || template?.content || '',
    });
  };
    ...
    <div className="modal-backdrop new-note-backdrop" onClick={onClose}>
      <div className="modal new-note-modal" onClick={(event) => event.stopPropagation()}>
        ...
        <label className="new-note-field new-note-field--full">
          <span>标题</span>
          <input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="输入标题" autoFocus />
        </label>

        <div className="new-note-grid">
          <label className="new-note-field">
            <span>笔记本</span>
            <select value={notebookId} onChange={(event) => setNotebookId(event.target.value)}>
              {notebooks.filter((item) => item.id !== 'all').map((notebook) => (
        ...
        <label className="new-note-field new-note-field--full">
          <span>标签（逗号分隔）</span>
          <input value={tagText} onChange={(event) => setTagText(event.target.value)} placeholder="如: Rust, 架构, 笔记" />
        </label>

        <label className="new-note-field new-note-field--full">
          <span>内容预览</span>
          <textarea value={content} onChange={(event) => setContent(event.target.value)} placeholder="可选的笔记内容...\n留空则使用模板内容" />
        </label>

        <div className="modal-actions new-note-actions">
          <button className="ghost-btn" onClick={onClose}>取消</button>
          <button className="primary-btn" onClick={createNote}>创建笔记</button>
        </div>
      </div>
    </div>
  );
