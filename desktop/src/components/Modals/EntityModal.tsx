  ...
  return (
    <div className="modal-backdrop new-note-backdrop" onClick={onClose}>
      <div className="modal new-note-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-actions new-note-actions"><button className="ghost-btn" onClick={onClose}>取消</button><button className="primary-btn" onClick={() => onConfirm(value.trim())}>{state.confirmText}</button></div>
      </div>
    </div>
  );
}
